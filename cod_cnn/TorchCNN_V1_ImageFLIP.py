...
transform_train = transforms.Compose([
    ...
    transforms.RandomHorizontalFlip(), #Flip orizontal aleatoriu pentru a creste diversitatea datelor de antrenament
    transforms.RandomVerticalFlip(), #Flip vertical aleatoriu pentru a creste diversitatea datelor de antrenament
...
class CNN(nn.Module):
    def __init__(self, num_classes=9, dropout=0.4): #dropout_rate este o tehnica de regularizare care ajuta la prevenirea overfitting-ului, 
        ...
        self.base = models.efficientnet_b3(weights=models.EfficientNet_B3_Weights.DEFAULT)
                        #Incarcam modelul EfficientNet-B3 preantrenat pe ImageNet, folosind greutatile implicite (DEFAULT) care au fost antrenate pe setul de date ImageNet.
        in_features = self.base.classifier[1].in_features  #in_features reprezinta numarul de caracteristici (features) care sunt produse de ultimul strat de clasificare al modelului EfficientNet-B3,
                                                        # inainte de a fi inlocuit cu noul strat de clasificare pentru cele 9 clase din setul nostru de date.
        self.base.classifier = nn.Sequential( #Inlocuim stratul de clasificare al modelului EfficientNet-B3 cu un nou strat care include dropout pentru regularizare si un strat linear pentru clasificare in cele 9 clase.
    ...
    def forward(self, x):
                            #.base se refera la modelul EfficientNet-B3 modificat, care include noul strat de clasificare cu dropout.
        return self.base(x) #In metoda forward, pur si simplu trecem inputul prin modelul EfficientNet-B3 modificat
                            # Acest lucru permite modelului sa invete sa clasifice imaginile in cele 9 clase din setul nostru de date, folosind reprezentarile invatate de EfficientNet-B3.
...
for param in model.base.features.parameters(): #Iteram prin toate straturile de caracteristici (features) ale modelului EfficientNet-B3, care sunt stocate in model.base.features, si setam requires_grad la False pentru fiecare parametru.
    param.requires_grad = False #In prima etapa, inghetam (freeze) toate straturile de caracteristici (features) ale modelului EfficientNet-B3, astfel incat doar stratul de clasificare (head) sa fie antrenat.
...
for epoch in range(num_epochs): #Incepem antrenarea propriu-zisa pentru num_epochs (10 epoci))
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), "malware_cnn_best.pth")
...
model.load_state_dict(torch.load("malware_cnn_best.pth", weights_only=True))
...
plt.title('Training Loss with Warmup')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()
plt.show()
