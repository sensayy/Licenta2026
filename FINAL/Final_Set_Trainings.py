...
#Comparam scorurile F1 pentru fiecare clasa intre modele
rects1 = ax.bar(x - width/2, rf_f1, width, label='Random Forest', color='blue') #Le asezam fata de centru pe toate
rects2 = ax.bar(x + width/2, xgb_f1, width, label='XGBoost', color='orange')
rects3 = ax.bar(x + width*1.5, lgbm_f1, width, label='LightGBM', color='green')
rects4 = ax.bar(x + width*2.5, svm_f1, width, label='SVM', color='red')
rects5 = ax.bar(x + width*3.5, knn_f1, width, label='KNN', color='purple')
...
#Log loss intre clase in comparatie pentru toate clasele
...
plt.bar(models, log_loss_values, color=['blue', 'orange', 'green', 'red', 'purple'])
