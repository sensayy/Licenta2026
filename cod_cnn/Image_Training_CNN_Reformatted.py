...
# =============================================================================
# MALWARE CLASSIFICATION - Custom CNN (Baseline)
# Microsoft BIG 2015 Malware Classification Challenge
# Grayscale byte visualization images → 9-class classifier
# Custom architecture with AdaptiveAvgPool to handle variable image heights
# =============================================================================
...
# =============================================================================
# CUSTOM DATASET
# Custom Dataset class to load malware images from disk using a labels dictionary
# built from the trainLabels.csv file provided with the BIG2015 dataset.
# =============================================================================
...
# =============================================================================
# MODEL DEFINITION - Custom CNN with AdaptiveAvgPool
# Uses AdaptiveAvgPool2d to collapse variable-height feature maps to a fixed
# size (4x4), allowing the network to handle images of different heights
# without requiring a fixed input resolution.
# =============================================================================
...
# =============================================================================
# DATA TRANSFORM
# PIL image → float tensor scaled to [0,1], then normalized to [-1,1]
# =============================================================================
...
# =============================================================================
# MAIN - Data Loading, Training Loop & Model Saving
# =============================================================================
if __name__ == "__main__":
    ...
    # -------------------------------------------------------------------------
    # Load labels from CSV and match against images available on disk
    # -------------------------------------------------------------------------
    df             = pd.read_csv("trainLabels.csv")
    existing_files = set(os.path.splitext(f)[0] for f in os.listdir("images/"))
    labels_dict    = {
        row["Id"]: row["Class"] - 1          # Class - 1 to convert from 1-indexed to 0-indexed
    ...
    # -------------------------------------------------------------------------
    # Train/Val Split - stratified to preserve class distribution
    # -------------------------------------------------------------------------
    ...
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.5) # Halves LR every 5 epochs
    ...
    # -------------------------------------------------------------------------
    # Save Model
    # Salvam modelul antrenat pentru a putea fi folosit ulterior pentru
    # inferenta sau pentru a continua antrenamentul daca este necesar.
    # -------------------------------------------------------------------------
    torch.save(model.state_dict(), "malware_cnn2.pth")
    print("Model saved to malware_cnn2.pth")